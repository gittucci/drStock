...
app = FastAPI(
    ...
    version="1.0",
    description="Esta é a API do microsserviço SUPPLIERS.",
...
@app.get("/")
async def root():
    return {"message": "Microsserviço SUPPLIERS rodando!"}

# Rota para verificação de saúde da aplicação
@app.get("/healthz")
def healthz():
    return {"mensagem": "A aplicação SUPPLIERS está saudável."}
...
async def startup_event():
    print("Iniciando o microsserviço SUPPLIERS ...")
    app.state.database = get_session() # tirei o await
    Base.metadata.create_all(bind=engine)
    logger.info("Tabelas do banco de dados criadas com sucesso.")
    logger.info("Inicialização da aplicação SUPPLIERS ...") 

async def shutdown_event():
    print("Encerrando o microsserviço SUPPLIERS ...")
    await app.state.database.disconnect()
    logger.info("Fechamento da aplicação SUPPLIERS ...") 
