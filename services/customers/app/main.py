...
app = FastAPI(
    ...
    version="1.0",
    description="Esta é a API do microsserviço CUSTOMERS.",
...
@app.get("/")
async def root():
    return {"message": "Microsserviço CUSTOMERS rodando!"}

# Rota para verificação de saúde da aplicação
@app.get("/healthz")
def healthz():
    return {"mensagem": "A aplicação CUSTOMERS está saudável."}
...
async def startup_event():
    print("Iniciando o microsserviço CUSTOMERS ...")
    app.state.database = get_session() # tirei o await
    Base.metadata.create_all(bind=engine)
    logger.info("Tabelas do banco de dados criadas com sucesso.")
    logger.info("Inicialização da aplicação CUSTOMERS ...") 

async def shutdown_event():
    print("Encerrando o microsserviço CUSTOMERS ...")
    await app.state.database.disconnect()
    logger.info("Fechamento da aplicação CUSTOMERS ...") 
